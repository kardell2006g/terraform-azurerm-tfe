...
variable "app_gw_sku_name" {
  ...
  validation {
    condition = (
      var.app_gw_sku_name == "Standard_v2" ||
      var.app_gw_sku_name == "WAF_v2"
    )

    error_message = "The app_gw_sku_name value must be 'Standard_v2' or 'WAF_v2'."
  }
}
...
variable "app_gw_sku_tier" {
  ...
  validation {
    condition = (
      var.app_gw_sku_tier == "Standard_v2" ||
      var.app_gw_sku_tier == "WAF_v2"
    )

    error_message = "The app_gw_sku_tier value must be 'Standard_v2' or 'WAF_v2'."
  }
}
...
variable "app_gw_waf_max_request_body_size_kb" {
  default     = 128
  type        = number
...
variable "app_gw_request_routing_rule_minimum_priority" {
  default     = 1000
  type        = number
  ...
  validation {
    condition = (
      var.app_gw_request_routing_rule_minimum_priority > 1 &&
      var.app_gw_request_routing_rule_minimum_priority < 19000
    )
...
variable "enable_boot_diagnostics" {
  type        = bool
  description = "Enable boot diagnostics for instances in the VMSS"
  default     = true
}
...
variable "postgres_flexible_source_server_id" {
  type        = string
  description = "The source postgres flexible server to restore from"
  default     = null
}
...
variable "sa_public_access_enabled" {
  type        = bool
  description = "Enable or disable public access on the TFE Storage Account."
  default     = false
}
...
variable "redis_rdb_backup_enabled" {
  type        = bool
  description = "Enable or disable Redis Database Backup (Creates point in time snapshots of dataset)."
  default     = true
}
...
variable "redis_subnet_id" {
  type        = string
  description = "Network subnet id for redis."
  default     = null
}

variable "redis_port" {
  type        = string
  description = "The port to access redis on. If ssl only access is enabled, the default port is 6380. The non-ssl defualt port is 6379."
  default     = "6380"
}

variable "enable_redis_authentication" {
  type        = bool
  description = "Whether or not to enable authentication to the redis cache. If set to false, the Redis instance will be accessible without authentication."
  default     = true
}

variable "enable_non_ssl_port" {
  type        = bool
  description = "Whether or not non-ssl can be used. Must be true if authentication is false."
  default     = false
}

variable "redis_min_tls_version" {
  type        = string
  description = "The Minimum TLS version to use when SSL authentication is used."
  default     = "1.2"
}